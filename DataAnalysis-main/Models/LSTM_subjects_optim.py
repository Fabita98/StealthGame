...
def train(model, subject_data, subject_labels, optimizer, criterion, epochs, window_size, subject_id, split_partitions=3):
    ...
    for epoch in range(epochs):
        ...
        train_dataloader = DataLoader(train_dataset, batch_size=window_size, shuffle=False, num_workers=8)
        ...
        for batch_features, batch_labels in train_dataloader:
            ...
            optimizer.zero_grad()
            batch_features = batch_features.unsqueeze(0).to(device)
            batch_labels = batch_labels.unsqueeze(0).to(device)
            output, hidden = model(batch_features, hidden)
            hidden = (hidden[0].detach(), hidden[1].detach())
            outputs.append(output.item())
            targets.append(batch_labels.item())
        ...
        val_dataloader = DataLoader(val_dataset, batch_size=window_size, shuffle=False)
...
def analize_subject(i):
    ...
    # Compilazione del modello (se PyTorch 2.0+)
